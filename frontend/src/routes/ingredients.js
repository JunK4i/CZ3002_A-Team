...
const Ingredients = ({ children }) => {
    ...
    const [categoryList, setCategoryList] = React.useState(["Category", "Dairy", "Meat", "Vegetable", "Fruit", "Grain", "Spice", "Other"]);
    ...
    const uid = "234"
    const defaultCategories = ["Dairy", "Meat", "Vegetable", "Fruit", "Grain", "Spice", "Other"]
    ...
    function renderError() {
        <div className="dialog">
            ...
            <div className="alert alert-danger" role="alert">
                Error
                <i className="x-button bi-x-square" onClick={e => {
                    setIsError(false);
                }}></i>
            </div>
        </div>
        ...
        return (
            <div className="dialog">
                ...
                <div className="dialog-card">
                    ...
                    <div className="white-card" style={{ marginTop: "10px", padding: "15px" }}>
                        <form onSubmit={(e) => { e.preventDefault(); }}>
                            <div className="form-group">
                                <label for="ingredient-name" className="text-bold">Ingredient Name</label>
                                <div class="input-group mb-3">
                                    <input id="ingredient-name" type="text" placeholder="Search Ingredient..." className="form-control" aria-label="Sizing example input" list="datalistOptions"
                                        aria-describedby="inputGroup-sizing-default" value={addSearchValue}
                                        onChange={e => { handleAddIngredient(e) }} onKeyDown={handleEnterSearch} required />
                                    <button className="btn btn-outline-secondary" type="button" id="button-addon1" onClick={handleClickSearch}><i className="bi-search" ></i></button>
                                    <datalist id="datalistOptions">
                                        {addSearchResults.map((item, index) => <option value={item.name} key={index}>{item.name}</option>)}
                                    </datalist>
                                </div>
                            </div>
                            ...
                            <div className="form-group" style={{ marginTop: "15px" }}>
                                ...
                                <input type="number" className="form-control" id="servings" placeholder="Enter Amount" value={addServings} onChange={(e) => setAddServings(e.target.value)} required />
                            </div>
                            <div className="form-group" style={{ marginTop: "15px" }}>
                                <label for="expiry-date" className="text-bold">Expiry Date</label>
                                <input type="date" className="form-control" id="expiry-date" placeholder="Enter Expiry Date" value={addExpiry} onChange={(e) => setAddExpiry(e.target.value)} required />
                            </div>
                            <div style={{ display: "grid", placeItems: "center", paddingTop: "15px" }}>
                                <button onClick={handleSubmitIngredient} type="submit" className="btn btn-warning" style={{ textSize: "10px", fontWeight: "bold", height: "100%", width: "60%" }}>
    ...
    function renderConsume() {
        ...
        return (
            <div className="dialog">
                <div className="dialog-mask"></div>
                <div className="dialog-card">
                    <i className="x-button bi-x-square" onClick={e => { setConsumeOpen(false); setConsumeServings(); }}></i>
    ...
    function renderEdit() {
        ...
        return (
            <div className="dialog">
                <div className="dialog-mask"></div>
                <div className="dialog-card">
                    <i className="x-button bi-x-square" onClick={e => { setEditOpen(false); setEditServings(); setEditExpiryDate() }}></i>
        ...
        <Container>
            ...
            <Row>
                ...
                <Col className="col-md-2 align-self-end flex justify-content-center">
                    <button onClick={e => { setAddOpen(true); }} type="button" className="btn btn-warning" style={{ textSize: "10px", fontWeight: "bold", height: "45%", width: "fit-content" }}>
                ...
                <Row style={{ padding: "15px 10px" }}>
                    ...
                    <Col className="col-md-5">
                        <div className="input-group">
                            ...
                            <select className="form-select" aria-label="Select Filter" onChange={(e) => handleFilterChange(e)}>
                                {categoryList.map((category, index) => <option value={category} key={index}>{category}</option>)}
                            </select>
                        </div>
                    </Col>
                    <Col className="align-self-center">
                        <Pagination currentPage={currentPage} totalCount={ingredients.length} pageSize={10} onPageChange={(handlePageChange)} />
                    </Col>
                </Row>
                <div className="ingredients-grid">
                    <div className="ingredients-row-wrapper">
                        <div>Ingredient</div> <div>Category</div> <div>Servings</div> <div>Date of Expiry</div> <div>Days to Expiry</div> <div><i className="bi-gear"></i></div>
                    </div>
                    {ingredients !== [] &&
                                            ...
                                            {optionsOpen[index] &&
                                                <div className="ingredients-options-wrapper">
                                                    <div className="ingredients-options">
                                                        <div className="ingredients-option" onClick={e => handleConsume(e, index)}>Consume</div>
                                                        <div className="ingredients-option" onClick={e => handleThrowAway(e, index)}>Throw Away</div>
                                                        <div className="ingredients-option" onClick={e => handleEditServings(e, index)}>Edit Servings</div>
                                                    </div>
                                                </div>
                                            }
                                        </div>
